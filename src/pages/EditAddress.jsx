...
const EditAddress = () => {
  ...
  const handleSearch = () => {
    ...
    const entregaEncontrada = entregasExistentes.find(
      (entrega) => entrega.nome === nome
    );
    if (entregaEncontrada) {
      setDados(entregaEncontrada);
      setTipoLocalizacao(
        entregaEncontrada.destino === "marte" ? "numerico" : "texto"
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    try {
      const entregasExistentes =
        JSON.parse(localStorage.getItem("entregas")) || [];
      const novaListaEntregas = entregasExistentes.filter(
        (entrega) => entrega.nome !== dados.nome
      );
      localStorage.setItem("entregas", JSON.stringify(novaListaEntregas));
  ...
  return (
    <div>
      ...
      <section id="edicao" className="editar conteiner">
        <div className="editar-conteudo">
          <h1 className="font-1-xxl cor-0">
            Edição de Endereço<span className="cor-p1">.</span>
          </h1>
          <div className="editar-search cor-0">
          ...
          {dados && (
            <form onSubmit={handleSubmit} className="form-container">
              ...
              <input
                ...
                value={dados.produto}
                onChange={(e) =>
                  setDados({ ...dados, produto: e.target.value })
                }
                required
              />
              ...
              <input
                ...
                onChange={(e) =>
                  setDados({ ...dados, dataEnvio: e.target.value })
                }
                required
              />
        ...
        <img
          src="https://images.pexels.com/photos/7170714/pexels-photo-7170714.jpeg"
          alt="Astronauta"
        />
      </section>
    </div>
